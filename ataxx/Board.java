...
class Board {
    ...
    /** Number of total open. */
    static final int TOTAL_OPEN = 45;
    /** Number of A. */
    static final int  A = 96;
    /** Number of B. */
    ...
    boolean legalMove(Move move) {
        PieceColor m = whoseMove();
        if (move == null) {
            return false;
        } else if (move.isPass()) {
            return !canMove(m);
        } else if (get(move.toIndex()) == EMPTY
                && get(move.fromIndex()) == m) {
            return true;
        }
        return false;
    }
    ...
    void makeMove(Move move) {
        ...
        if (move.isExtend()) {
            ...
            for (int m = -1; m <= 1; m++) {
                for (int n = -1; n <= 1; n++) {
                    if (get((char) (move.col1() + m),
                            (char) (move.row1() + n))
                            == whoseMove().opposite()) {
                        set((char) (move.col1() + m),
                                (char) (move.row1() + n), whoseMove());
                        _numPieces[whoseMove().ordinal()]++;
    ...
    void detWin() {
        if (_numJumps >= JUMP_LIMIT
                || (!canMove(whoseMove())
                && !canMove(whoseMove().opposite()))
            || _numPieces[whoseMove().ordinal()] == 0
                || _numPieces[whoseMove().opposite().ordinal()] == 0) {
    ...
    void undo() {
        while (_undoPieces.peek() != null) {
            int currSquare = _undoSquares.pop();
            PieceColor currPiece = _undoPieces.pop();

            if (get(currSquare) == EMPTY) {
                if (currPiece != EMPTY) {
                    _totalOpen++;
                    _numPieces[currPiece.ordinal()]++;
                }
            } else if (get(currSquare) != EMPTY) {
                if (currPiece != get(currSquare)) {
                    _numPieces[currPiece.ordinal()]++;
                    _numPieces[get(currSquare).ordinal()]--;
                }
            }

            unrecordedSet(currSquare, currPiece);
        }
        _undoPieces.pop();
        _undoSquares.pop();
    ...
    void setBlock(char c, char r) {
        ...
        unrecordedSet((char) (midCol + disCol),
                (char) (midRow + disRow), BLOCKED);
        unrecordedSet((char) (midCol - disCol),
                (char) (midRow - disRow), BLOCKED);
        unrecordedSet((char) (midCol + disCol),
                (char) (midRow - disRow), BLOCKED);
    ...
    /** stack for numJumps. **/
    private Stack<Integer> _numJumpTrack;
